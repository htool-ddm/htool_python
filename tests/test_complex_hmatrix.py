...
class GeneratorSubMatrix(Htool.ComplexVirtualGenerator):
    ...
    def build_submatrix(self, J, K, mat):
        for j in range(0, len(J)):
            for k in range(0, len(K)):
                mat[j, k] = (1.0+1j*np.sign(J[j]-K[k])) / (1.e-5 + np.linalg.norm(
...
def test_Complex_HMatrix(NbRows, NbCols, Symmetric, UPLO):
    ...
    HMatrix = Htool.ComplexHMatrix(
        cluster_target, cluster_source, epsilon, eta, Symmetric, UPLO)
