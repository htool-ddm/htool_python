...
def FactoryHMatrix(Generator, points_target, points_source, Symmetric):
    if Symmetric:
        return Htool.HMatrix(Generator, points_target, True)
    else:
        return Htool.HMatrix(Generator, points_target, points_source)


@pytest.mark.parametrize("GeneratorType,NbRows,NbCols,Symmetric", [
    ("Coef",500, 500, True),
    ("Coef",500, 500, False),
    ("Coef",500, 250, False),
    ("SubMatrix",500, 500, True),
    ("SubMatrix",500, 500, False),
    ("SubMatrix",500, 250, False),
])
def test_HMatrix(GeneratorType, NbRows, NbCols, Symmetric):
    ...
    # Build
    HMatrix = FactoryHMatrix(Generator, points_target, points_source,Symmetric)
