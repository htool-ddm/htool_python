...
void declare_DDM(py::module &m, const std::string &className) {
    ...
    using ClassDense = DDM<CoefficientPrecision, HPDDM::LapackTRSub>;
    py::class_<ClassDense> py_class_dense(m, (className + "Dense").c_str());
    py_class_dense.def("facto_one_level", &ClassDense::facto_one_level);
    if constexpr (!htool::is_complex<CoefficientPrecision>()) {
        py_class_dense.def("build_coarse_space", py::overload_cast<VirtualCoarseSpaceBuilder<CoefficientPrecision> &, VirtualCoarseOperatorBuilder<CoefficientPrecision> &>(&ClassDense::build_coarse_space));
    }
    py_class_dense.def(
        "solve", [](ClassDense &self, py::array_t<CoefficientPrecision, py::array::f_style> x, const py::array_t<CoefficientPrecision, py::array::f_style | py::array::forcecast> b, std::string hpddm_args) {
            // HPDDM arguments
            HPDDM::Option &opt = *HPDDM::Option::get();
            opt.parse(hpddm_args);
            int mu;
        ...
        py::arg("b"),
        py::arg("hpddm_args") = "");
    py_class_dense.def("set_hpddm_args", [](ClassDense &self, std::string hpddm_args) {
        HPDDM::Option &opt = *HPDDM::Option::get();
        opt.parse(hpddm_args);
    });
    py_class_dense.def("get_information", py::overload_cast<>(&ClassDense::get_information, py::const_));
}
