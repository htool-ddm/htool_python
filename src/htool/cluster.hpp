...
void declare_Cluster(py::module &m, const std::string &className) {
    py::class_<Cluster<ClusterType>, std::shared_ptr<Cluster<ClusterType>>>(m, className.c_str())
        .def(py::init<>())
        .def(
            "build", [](Cluster<ClusterType> &self, const std::vector<R3> &x, const std::vector<double> &r, const std::vector<int> &tab, const std::vector<double> &g, int nb_sons, MPI_Comm_wrapper comm) {
        ...
        .def(
            "display", [](Cluster<ClusterType> &self, const std::vector<R3> &x, int depth, MPI_Comm_wrapper comm) {
                int rankWorld;
