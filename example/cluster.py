...
# Cluster 3D
cluster = Htool.BoundingBox1RegularClustering(3)
...
r = np.random.uniform(low=0, high=1, size=Number_points)  # radius
theta = np.random.uniform(low=0, high=2*np.pi, size=Number_points)  # angle

points_2D[0, :] = np.sqrt(r) * np.cos(theta)
points_2D[1, :] = np.sqrt(r) * np.sin(theta)

# Cluster 2D
cluster = Htool.PCAGeometricClustering(2)
cluster.set_minclustersize(minclustersize)
